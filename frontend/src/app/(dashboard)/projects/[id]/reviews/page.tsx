...
export default function ProjectReviewsPage() {
  ...
  const getTemplateName = (template: string) => {
    switch(template) {
      case 'ARCHITECTURE': return 'Architecture Analysis';
      case 'DOCUMENTATION': return 'Documentation Generator';
      default: return template.replace('_', ' ');
    }
  };
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, gap: 12 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, letterSpacing: '-0.5px' }}>Code Reviews & AI Analysis</h1>
        <button className="btn-primary" onClick={() => setShowCreate(true)} disabled={providers.length === 0}>
          {providers.length === 0 ? 'Add AI Provider First' : 'New Analysis'}
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="card glass-panel" style={{ padding: 40, textAlign: 'center' }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>No reviews yet</h3>
          <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginBottom: 16 }}>Run your first AI-powered code review or analysis.</p>
          {providers.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {reviews.map(review => (
            <Link key={review.id} href={`/reviews/${review.id}`} className="card glass" style={{ padding: 16, textDecoration: 'none', color: 'inherit' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>
                    {getTemplateIcon(review.template)} {getTemplateName(review.template)}
                  </span>
...
function CreateReviewDialog({ projectId, files, providers, onClose, onCreated }: {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content card glass-panel animate-fade-in" onClick={e => e.stopPropagation()}>
        <div className="modal-header" style={{ padding: '0 0 20px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700 }}>Run Code Review or Analysis</h2>
        </div>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div className="form-group">
            <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 6, display: 'block' }}>Review Template</label>
            <select className="input-field" value={template} onChange={e => setTemplate(e.target.value)}>
              <option value="CODE_QUALITY">✨ Code Quality</option>
              <option value="SECURITY">🔒 Security</option>
              <option value="PERFORMANCE">⚡ Performance</option>
              <option value="ARCHITECTURE">🏗️ Architecture Analysis (Bonus)</option>
              <option value="DOCUMENTATION">📚 Documentation Generator (Bonus)</option>
            </select>
          </div>
          <div className="form-group">
            <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 6, display: 'block' }}>Scope</label>
          ...
          {scope !== 'PROJECT' && (
            <div className="form-group">
              <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 6, display: 'block' }}>Select Files</label>
              <div style={{ maxHeight: 200, overflow: 'auto', border: '1px solid var(--border-color)', borderRadius: 8, padding: 8, background: 'rgba(0,0,0,0.2)' }}>
                {files.map(f => (
          ...
          <div className="form-group">
            <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 6, display: 'block' }}>AI Provider</label>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 8, paddingTop: 20, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            <button type="button" className="btn-secondary" onClick={onClose}>Cancel</button>
            <button type="submit" className="btn-primary" disabled={isSubmitting}>
              {isSubmitting ? <><div className="spinner" style={{width:16, height:16, borderWidth: 2}} /> Processing...</> : 'Run Analysis'}
            </button>
          </div>
        </form>
      </div>
